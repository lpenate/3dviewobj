<template>
  <Card size="sm">
    ...
    <CardContent class="flex flex-col gap-3">
      <!-- Móvil / tablet: la carga local solo está disponible en escritorio. -->
      <div
        v-if="disabled"
        class="flex flex-col items-center gap-2 rounded-xl border border-dashed border-input bg-muted/40 px-4 py-5 text-center"
        aria-disabled="true"
      >
        <span
        ...
        <p class="text-sm font-medium text-muted-foreground">
          Carga de modelos locales no disponible
        </p>
        <p class="text-xs text-muted-foreground">
          La versión de escritorio permite arrastrar o seleccionar ficheros .glb
          y .gltf de tu equipo.
        </p>
      </div>
      ...
      <template v-else>
        ...
        >
          ...
          <p class="text-xs text-muted-foreground">
            Los recursos externos de un .gltf se pueden soltar a la vez o
            después
          </p>
...
import { ref } from 'vue'
import { MonitorIcon, UploadIcon, WatchIcon } from '@lucide/vue'
...
defineProps<{
  /** Deshabilita la carga de ficheros locales (móvil / tablet). */
  disabled?: boolean
}>()
